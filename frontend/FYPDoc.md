...
*   **Key Features**:
    *   **Real Authentication System**: Implement JWT-based authentication for secure session management.
    *   **Role-Based Access**: Separation of standard User and Admin privileges at the route and API level.
    *   **Email Verification**: (Optional/High Impact) Security step requiring users to verify their email before engaging in trades.
    *   Password hash comparison and logout functionality.
...
*   **Key Features**:
    *   **Image Gallery & Upload System**: Robust backend file upload API integrating with local or cloud storage (e.g., Cloudinary), replacing mock URLs.
...
*   **Key Features**:
    ...
    *   **Backend Search Optimization**: Advanced keyword search, category filtering, location-based (Geo-location) filtering, and sorting (latest, value, relevance).
...
*   **Key Features**:
    *   **AI Valuation API Integration**: Backend endpoint connecting to a real (or mocked) AI service to process item details and estimate LTP range based on market data.
    *   **Smart Match Algorithm**: Backend rule-based algorithm that continuously maps user searches/wishlists against new items to generate high-probability trade alerts.
    *   **Similar Items**: On any PDP, the system displays "You might also like" items.
...
#### 3.2.7 Bidding, Negotiation & Trade Escrow
*   **Functionality**: The transaction execution and negotiation layer, acting as a secure escrow mechanism.
*   **Key Features**:
    *   **Offer Negotiation System**: Users can propose robust offers including counter-offers, multi-item offers, item + LTP combinations, and offer expiration timers.
    *   **Trade Lifecycle Management (Escrow)**: Trades move through strict state validations safeguarding both parties: *Pending -> Accepted -> In Progress -> Completed -> Disputed*.
    *   **Bid Management**: Sellers can view and manage all incoming offers in a centralized dashboard.
    *   **Trade Completion**: Both parties securely mark the trade as "Completed" to trigger inventory updates and reputation scoring.
...
*   **Functionality**: Platform oversight and dispute resolution.
*   **Key Features**:
    *   **User Management**: List of all registered users with "Ban/Suspend" controls.
    *   **Admin Dispute Handling**: Review system for trades marked as "Disputed", allowing admins to evaluate evidence (chat logs, transaction history) and resolve conflicts.
    *   **Analytics & Fraud Detection**: Visual graphs showing daily signups and active trades, with administrative fraud detection flags for suspicious accounts.

#### 3.2.10 Trust & Reputation System
*   **Functionality**: Ensuring platform safety and building community trust.
*   **Key Features**:
    *   **Rating & Reviews**: Mandatory post-trade rating system (1–5 stars) with written review comments for both buyers and sellers.
    *   **Trust Score Algorithm**: Backend calculation logic to determine a user's reliability index based on successful trades and average rating.

#### 3.2.11 Notification System
*   **Functionality**: Database-driven or real-time (SignalR) alerts for critical user actions.
*   **Key Features**:
    *   **Alerts**: Instant notifications for New Messages, Offers Received, Offers Accepted/Rejected, and Smart Match recommendations.
