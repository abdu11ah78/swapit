...
export function ItemDetailPage() {
  ...
  const handleBidAction = async () => {
    ...
    if (Number.isNaN(Number(bidAmount)) || Number(bidAmount) <= 0) {
    ...
    try {
      ...
      setBids([newBid, ...bids])
      setBidAmount("")
      toast.success(`Offer submitted: ${newBid.amount} LTP`)
    } catch {
      toast.error("Offer API unavailable. Please try again.")
    }
  }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 p-8 rounded-[2rem] bg-slate-900 border border-slate-800">
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {createTradeMutation.isPending || createOfferMutation.isPending ? "Submitting..." : "Place Bid"}
                    </button>
                  </div>
                  <textarea
                    className="mt-4 w-full bg-slate-950 border border-slate-800 rounded-2xl py-3 px-4 text-xs font-medium text-white focus:outline-none focus:border-[#115e59]"
                    rows={2}
                    value={offerMessage}
